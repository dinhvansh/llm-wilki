...
async def get_settings(db: Session = Depends(get_db), actor: Actor = Depends(require_roles("admin"))):
    return serialize_runtime_settings(db)


@router.put("/settings", response_model=SettingsResponse)
async def save_settings(payload: SettingsPayload, db: Session = Depends(get_db), actor: Actor = Depends(require_roles("admin"))):
...
async def test_settings_connection(payload: TestConnectionPayload, actor: Actor = Depends(require_roles("admin"))):
